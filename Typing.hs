...
freshVar x = (Generic_ ("a" ++ show (x + 1)), x + 1)
...
instance Substitute FunDecl where
	substitute a b (FD ret name args vars stmts) = FD (substitute a b ret) name (substitute a b args) (substitute a b vars) (substitute a b stmts)
